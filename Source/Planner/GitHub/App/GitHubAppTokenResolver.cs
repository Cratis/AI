...
[Singleton]
public class GitHubAppTokenResolver(IGitHubAppClient appClient, IMongoCollection<GitHubAppInstallation> installations) : IGitHubAppTokenResolver
{
    /// <inheritdoc/>
...
public class GitHubAppNotInstalled(OrganizationName organization)
    : Exception($"The GitHub App is not installed for '{organization.Value}', and no single installation to fall back to is configured.");
