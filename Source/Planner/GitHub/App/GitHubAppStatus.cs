...
/// <param name="Organization">The organization the App is registered under and installed into - empty for a personal App.</param>
[ReadModel]
public record GitHubAppStatus(bool IsConfigured, string Slug, string Name, string Organization)
{
    /// <summary>
    ...
    public static GitHubAppStatus Current(IOptions<GitHubAppOptions> options) =>
        new(options.Value.IsConfigured, options.Value.Slug, options.Value.Name, options.Value.Organization);
}
