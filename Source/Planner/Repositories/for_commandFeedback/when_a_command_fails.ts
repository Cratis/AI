...
describe('when a command fails', () => {
    ...
    it('should report the validation messages when the command is invalid', () => {
        const result: ICommandResult = {
            ...successfulResult,
            isValid: false,
            validationResults: [new ValidationResult(ValidationResultSeverity.Error, 'An organization name is required', [], undefined)],
        };
        commandFailureMessage(result)!.should.equal('An organization name is required');
    });
