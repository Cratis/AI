...
The Chronicle and Studio MCP guidance candidates are passive Agent Skill
content, not Agent Plugins MCP components. Their independent deny-all
classification catalogs generate Markdown references, but create no `mcp`
manifest entry, server configuration, transport, dependency, or executable
byte. Product evidence cannot cross-authorize another product. Effectful
guidance and local or remote MCP server classes remain non-emitting.
...
node tooling/generate-mcp-guidance-references.mjs
...
node tooling/mcp-guidance-validation.mjs
