...
| #339 | `feat/provider-crud` | First real migration slice: `Providers/Adding/` and `Reconfiguring/` (all 5 vendors, including "blank keeps existing" semantics), `Renaming/`, `Removing/`, `ConfiguredAIProvider` upgraded from placeholder record to a real `[ReadModel][Passive]` projection - a complete provider CRUD surface; decision 0006 |
