...
>
> `Cratis/AI.Distribution` becomes a CI-published mirror that is later archived,
> so the already published `v0.1.0` through `v0.3.0` install commands and the
> evidence records citing its tags keep resolving. Its tags are never deleted.
> The governed S9/S10 release lane still targets it and is separately blocked;
> nothing in that lane changed.
...
Generated artifacts are bot-authored. The public marketplace release is written
by CI to the protected `distribution` branch of `Cratis/AI` and tagged
`dist/vX.Y.Z`; the fixture and candidate-review lanes still open bot pull
requests against `Cratis/AI.Distribution`. Humans review generated output but do
not edit generated bytes in either place.

Both generated surfaces carry one narrow control-plane exception for the exact
verification workflow and dependency-free validator. Their canonical source
remains `distribution/repository-control-plane/` in `Cratis/AI`, they stay
outside package manifests and checksums, and they are preserved when generated
payload bytes are replaced. That exception does not make a generated ref or a
generated repository an authoring surface, and humans still do not patch
control-plane or generated artifact bytes in place.
