...
| Concern | Owner today | Planned owner — Cratis/AI#264 | Examples |
| --- | --- | --- | --- |
| Shared AI behavior | `Cratis/AI` (authored) | `Cratis/AI` (authored, unchanged) | Skill workflows, trigger intent, engineering conventions, profile composition, MCP declarations |
| Product facts | Owning product repository | Owning product repository (unchanged) | Arc APIs, Chronicle semantics, Components examples, supported client versions |
| Project context | Consuming repository | Consuming repository (unchanged) | Product mix, profile, credentials, endpoints, local constraints |
| Generated artifacts | `Cratis/AI.Distribution` | `Cratis/AI` at a generated ref | Immutable host packages, manifests, checksums, provenance |

> **Planned — [Cratis/AI#264](https://github.com/Cratis/AI/issues/264).** The
> third column is a recorded decision, not current behavior. Today the release
> pipeline publishes to `Cratis/AI.Distribution`, and that remains correct.
> The consolidation moves generated artifacts to a protected, CI-written ref
> inside `Cratis/AI` with immutable `dist/vX.Y.Z` tags, and reduces
> `Cratis/AI.Distribution` to a CI-published mirror that is later archived so
> the already published install commands and the evidence records citing its
> tags keep resolving. Nothing in this document's commands changes until that
> work lands; see the
> [maintainer marketplace deployment runbook](./maintainer-marketplace-deployment-runbook.md)
> for what a maintainer must configure by hand.

The four owners do not change with consolidation. Only the *location* of the
generated artifacts does: the boundary that keeps generated bytes machine-written
becomes a ref and a subtree instead of a second repository.
