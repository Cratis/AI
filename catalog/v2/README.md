...
S10 release policy, prerequisite approvals, control attestations, and marketplace
publication contracts remain authored under `distribution/`. Evidence baseline
identity is authored at `../evidence-baseline.json`.
...
`release-readiness.json` is generated by
`node tooling/generate-release-readiness.mjs`. It is currently `BLOCKED` with
every grant false and is consumed by the human catalog and workflow preflight.
