...
description: >
  ...
  Receives a high-level goal, breaks it into parallelizable tasks,
...
- **Backend before frontend** — TypeScript proxies are generated by `dotnet build -c Debug`; frontend cannot start until backend is compiled.
- **Specs after backend** — specs depend on the slice file existing and compiling, and are **mandatory for every slice type** (not just State Change).
- **Build is a synchronization point** — `dotnet build -c Debug` must succeed before any frontend or spec work begins.
