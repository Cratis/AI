...
Default to plain Markdown. Use MDX only when an imported component materially improves comprehension. Follow the rendering rule for the exact component import form. The raw Markdown mirror behind page actions exposes synchronized Markdown/MDX, so imports and JSX remain visible even when the rendered page looks polished.
...
- accurate, source-verified APIs and extension receivers;
- complete examples and visible outcomes;
- meaningful `title` and `description`, with existing frontmatter preserved during edits;
