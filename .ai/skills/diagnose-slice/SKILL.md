...
| Reactor's returned events never appended | the returned value is not a side-effect type Chronicle recognizes (unregistered `[EventType]`, or a custom type no `IReactorSideEffectHandler` handles) — it is discarded with a warning | return a registered event type, `EventForEventSourceId`, or an `IEnumerable` of either — `reactors.md` |
