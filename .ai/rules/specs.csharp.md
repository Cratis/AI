...
- **Prefer a concept's sentinel over a raw empty primitive.** When a `ConceptAs<T>` / `EventSourceId<T>` value is expected — as an argument, a mock setup, or an assertion — use the sentinel the type actually declares — usually `NotSet`, sometimes `New()` — rather than a `string.Empty` / `Guid.Empty` / `0` that implicitly converts. Check the type before reaching for a name: sentinels are not uniform across the framework (`TenantId` declares `NotSet` and `Default` but no `Empty`, while its neighbor `TenantName` declares `Empty`), so a guessed sentinel is a compile error. It states intent and survives a sentinel-value change (see [concepts.md](./concepts.md)). Reserve raw `string.Empty` for genuinely non-concept `string` values (e.g. asserting a decrypted-to-empty JSON payload).
