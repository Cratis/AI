...
- **Side-effect events:** return them directly as event objects (single `TEvent`, `IEnumerable<object>`, or `Task<...>` thereof) — Chronicle appends them to the triggering event source. To target a different event source, or to set the stream type/id, source type, `Subject`, `Occurred` or `Tags` per event, return `EventForEventSourceId(id, @event)` wrappers — the same cross-stream mechanism as a command (see [reactors.md](./reactors.md)). The target *event sequence* is not settable from a return value. Don't inject `IEventLog` for the normal path; reserve it for explicit append-result/wait/options/mixed-target workflows.
