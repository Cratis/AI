...
- Commands/queries are **model-bound**: a `[Command]` record with `Handle()` **on the record**, and `[ReadModel]` records with **static** query methods. The marker/handler interfaces `ICommand`, `ICommandHandler<T>`, `IQuery<T>`, `IQueryHandler<T,R>` **do not exist** — never use them. Arc does declare a **non-generic** `Cratis.Arc.Commands.ICommandHandler`, but it is an internal pipeline abstraction (`CommandType`, `Dependencies`, `Handle(CommandContext)`) that Arc builds for you through `ICommandHandlerProvider` — an application never implements it, and it never belongs in an example.
- Bootstrap has **three host shapes** — don't mix them. **ASP.NET Core** (`Cratis.Arc`): `WebApplication.CreateBuilder(args)` → `builder.AddCratisArc()` → `app.UseCratisArc()` → `app.Run()`. **Arc.Core self-hosted** (`Cratis.Arc.Core`, no ASP.NET underneath): `ArcApplication.CreateBuilder(args)` (**not** `ArcApplicationBuilder.CreateBuilder`) → `builder.AddCratisArc()` → `app.UseCratisArc()` → `await app.RunAsync()`. **Full stack** (`Cratis` metapackage — Arc + Chronicle + Microsoft Identity Platform): `WebApplication.CreateBuilder(args)` → `builder.AddCratis()` → `app.UseCratis()` → `app.Run()`. Every `Use*` here takes **no args**.
- The shapes are **not interchangeable**. `ArcOptions.Hosting.ApplicationUrl` sets the listen URL on the **Arc.Core path only** — it is read in exactly one production site, `ArcApplication`'s `GetHttpPrefixes()`; under ASP.NET the URL is Kestrel's. `AddCratis`/`UseCratis` extend `WebApplicationBuilder`/`IApplicationBuilder`, and `ArcApplicationBuilder` is neither, so writing them on the `ArcApplication.CreateBuilder` path is a **compile error** (the method is not in scope) — it does **not** "throw". Attach Chronicle there with `AddCratisArc(configureBuilder: arc => arc.WithChronicle())`, which binds the `Cratis.Arc.Chronicle` overload; the metapackage's same-named `WithChronicle` casts to `WebApplicationBuilder`.
