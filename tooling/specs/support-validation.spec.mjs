...
test("all 150 observations, 172 fact IDs, 11 legacy gaps, 108 official sources, and 14 distribution evidence files are accounted exactly", () => {
    const catalogs = loadSupportCatalogs();
    assert.equal(catalogs.evidence.observations.length, 150);
...
test("coordinated observation and reciprocal claim ID mutation fails the independent evidence anchor", () => {
    ...
    hasError(
        validateNormalizedEvidence(catalogs),
        "preserve all 150 S0-S7 evidence IDs exactly once",
    );
});
