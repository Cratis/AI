...
test("passive candidate assets package every currently safe target and account for exclusions", () => {
    ...
    withTemporaryDirectory((root) => {
        ...
        assert.equal(publicManifest.targetIds.length, 46);
        assert.equal(publicManifest.sourceSkills.length, 46);
        ...
        assert.equal(skillComponentIds.length, 83);
        ...
        for (const manifest of [publicManifest, engineeringManifest]) {
            ...
            assert.equal(coverage.componentCount, 179);
            assert.equal(
...
test("candidate component coverage closes every modeled component kind", () => {
    ...
    assert.equal(coverage.records.length, 179);
    assert.equal(
