...
test("catalog v2 preserves all 56 sources while split and merge targets are independent", () => {
    const catalogs = loadCatalogs();
    assert.equal(catalogs.sources.sources.length, 56);
    assert.equal(catalogs.targets.targets.length, 56);
...
test("the accepted Option A+ decision still blocks unapproved live targets", () => {
    ...
    assert.equal(publicCandidate.componentInventory.skills.length, 46);
    assert(
