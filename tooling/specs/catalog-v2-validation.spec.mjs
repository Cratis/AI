...
test("catalog v2 preserves all 49 sources while split and merge targets are independent", () => {
    const catalogs = loadCatalogs();
    assert.equal(catalogs.sources.sources.length, 49);
    assert.equal(catalogs.targets.targets.length, 49);
...
test("the accepted Option A+ decision still blocks unapproved live targets", () => {
    ...
    assert.equal(publicCandidate.componentInventory.skills.length, 38);
    assert(
