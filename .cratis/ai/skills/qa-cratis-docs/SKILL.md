---
name: qa-cratis-docs
description: Use this skill whenever Cratis documentation must be visually reviewed or a docs render/gate problem must be diagnosed: screenshots, light/dark appearance, raw table pipes, unstyled callouts, literal MDX imports, missing sidebar pages, broken Astro builds, stale previews, Mermaid failures, layout shift, or flicker. It verifies the authored source, synced output, rendered HTML, emitted Markdown, and final pixels without guessing.
---

# Diagnose and visually verify Cratis documentation

Product docs render through the sibling `Documentation/web` Astro Starlight site. A source file can pass Markdown lint while silently rendering the wrong HTML, so inspect each boundary: authored source → synced source → built HTML and emitted Markdown → screenshot.

## 1. Reproduce from a fresh site state

Run the owning repository's local documentation gate when present, then the sibling site gate:
...
Do not invent or require the example local command in a repository that does not provide it. The full check requires the Documentation checkout and available sibling products. Separate failures caused by unrelated sibling content or skipped optional tools from failures in the page under review.
...
| Untitled or unstyled callout | The only directive variants are `note`, `tip`, `caution`, and `danger`; an unknown name silently becomes a plain `<div>` |
| Literal `import …` text or inert component tag | JSX was authored in `.md`; rename and rewire it deliberately as `.mdx` |
| Shared component cannot resolve | Use an exact default import from `@components/<Name>.astro`; there is no bare `@components` barrel |
...
| Real landing only at `/overview/` | Look for a `<folder>.md` plus `<folder>/index.md[x]` legacy route collision |
| Empty icon | Verify the installed Starlight icon name; several icon failures are silent (`seti:windows`, not `windows`) |
...
| Rendered page looks correct but “Copy Markdown” is noisy | Inspect the emitted `.md`; MDX imports and JSX remain visible in the raw Markdown mirror |
...
Replace the example route with the actual product route. Read both PNGs. Crop with the installed `sharp` package when the relevant section is small:
...
Report the authored source, synced output, built HTML, emitted Markdown, and screenshot evidence separately. A green build is not visual proof.
