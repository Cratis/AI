...
now stages, verifies, pushes, tags, and releases inside this repository. What is
left is configuration a maintainer applies by hand: creating and protecting the
orphan `distribution` branch and the `dist/*` tag namespace, and dispatching the
workflow once to produce the first `Cratis/AI`-hosted release. See the
...
branch carries only thin pointer marketplace manifests that resolve the immutable
tag — never the authored root `skills/` tree.
...
branch at an immutable `dist/vX.Y.Z` tag. Its commands are:

```text
/plugin marketplace add Cratis/AI#dist/vX.Y.Z
/plugin install public-cratis-ai@cratis
```

```bash
codex plugin marketplace add Cratis/AI --ref dist/vX.Y.Z
copilot plugin marketplace add Cratis/AI#dist/vX.Y.Z
gemini extensions install https://github.com/Cratis/AI --ref dist/vX.Y.Z
pi install git:github.com/Cratis/AI@dist/vX.Y.Z
```

`/plugin marketplace add Cratis/AI` also works: this repository's default branch
carries thin pointer marketplace manifests that resolve the same tag. Each
