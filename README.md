...
- `.github/workflows/publish.yml` - Uses `cratis/release-action@v1` for automated version/release handling, then builds `cratis-plugin.tgz` and `cratis-plugin.zip`, attaches them (plus marketplace metadata) to the generated release, and publishes to configured Claude/Copilot marketplace endpoints.
