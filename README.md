...
| Concern | Owner today | Planned owner — Cratis/AI#264 |
| --- | --- | --- |
| Shared AI behavior and profile composition | `Cratis/AI` | `Cratis/AI` (unchanged) |
| Product APIs, versions, and facts | The owning Cratis product repository | The owning Cratis product repository (unchanged) |
| Repository-specific context | The consuming repository | The consuming repository (unchanged) |
| Generated immutable packages | `Cratis/AI.Distribution` | `Cratis/AI` at a generated ref — a protected, CI-written branch and immutable `dist/vX.Y.Z` tags |
...
right-hand column is a recorded decision, not current behavior. Today the
release pipeline still publishes to `Cratis/AI.Distribution`, and that is
correct. The consolidation moves the publish target into a generated ref of this
repository and reduces `Cratis/AI.Distribution` to a CI-published mirror that is
later archived, so the already published install commands and the evidence
records citing its tags keep resolving. Executing it needs branch protection and
GitHub App changes a maintainer must make deliberately; see the
[maintainer marketplace deployment runbook](Documentation/maintainer-marketplace-deployment-runbook.md).
The anti-drift guarantee is unchanged by the move: generated bytes stay
machine-written, protected, and never hand-edited.
