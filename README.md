...
| Generated immutable packages | `Cratis/AI` at a generated ref | The protected, CI-written `distribution` branch and immutable `dist/vX.Y.Z` tags |
| Already published `v0.1.0`–`v0.3.0` | `Cratis/AI.Distribution` | A CI-published mirror, later archived; its tags are never deleted |
...
now stages, verifies, pushes, and releases inside this repository, on a push to
`main` with the version computed by `cratis/release-action` like every other
Cratis release. What is left is configuration a maintainer applies by hand:
creating and protecting the orphan `distribution` branch and the `dist/*` tag
namespace, and letting one labeled push produce the first `Cratis/AI`-hosted
release. See the
[maintainer marketplace deployment runbook](Documentation/maintainer-marketplace-deployment-runbook.md).
The governed S9/S10 release lane is a separate, still-blocked path and is not
affected. The anti-drift guarantee is unchanged by the move: generated bytes stay
machine-written, protected, and never hand-edited, and this repository's default
branch carries only thin pointer marketplace manifests that resolve an exact,
immutable `distribution` branch commit — never the authored root `skills/` tree.
...
**Planned — [Cratis/AI#264](https://github.com/Cratis/AI/issues/264).** A
`generated/` tree and thin root pointer manifests join this layout when the
publish target moves into this repository. They do not exist yet, and no
generated host tree is committed here today.
...
is the latest **published** marketplace, and its commands keep working. The
corrected release policy admits 29 public skills and excludes five effectful
workflows, including live Chronicle operations, until their effects are
separately assessed and approved. It provides one `public-cratis-ai` root for
Agent Skills, Agent Plugin, Claude Code, Codex, GitHub Copilot, Gemini CLI, Kiro,
and Pi. Claude, Codex, Copilot, and Pi public install/discovery/remove canaries
pass; Gemini gallery discovery is enabled. OpenAI and Cursor packages are
prepared release assets but still require owner-authenticated vendor submission
and review.

The **next** release is hosted here instead, on the protected `distribution`
branch. The pin is the exact commit that release pushed to that branch —
`<distribution-sha>` below, recorded in the `dist/vX.Y.Z` GitHub Release and in
the pointer manifests. The `distribution` branch itself is never tagged; the tag
is the versioned record on `main`. Its commands are:

```text
/plugin marketplace add Cratis/AI#<distribution-sha>
/plugin install public-cratis-ai@cratis
```

```bash
codex plugin marketplace add Cratis/AI --ref <distribution-sha>
copilot plugin marketplace add Cratis/AI#<distribution-sha>
gemini extensions install https://github.com/Cratis/AI --ref <distribution-sha>
pi install git:github.com/Cratis/AI@<distribution-sha>
```

`/plugin marketplace add Cratis/AI` also works: this repository's default branch
carries thin pointer marketplace manifests that resolve the same commit. Each
release's own `README.md` carries the exact version-pinned commands. The code for
this is merged; the first real run waits on the maintainer creating and
protecting the branch — see the
[maintainer marketplace deployment runbook](Documentation/maintainer-marketplace-deployment-runbook.md).
